...
def validate_asterisk_number(text):
    # Regular expression to match **number** where the number is between 0 and 100
    ...
    if match:
        ...
        raise ValueError("The input string does not match the required format (**number**).")
...
# %%
forecasting_question = "Will global average temperatures be more than 2 degrees Celsius above pre-industrial levels by the year 2050?"
...
for i in range(single_model_repetitions):
    response = openai.chat.completions.create(
        ...
        messages = [
            {
                "role": "user",
                "content": f"{forecasting_question}"
            },
            {
                "role": "system",
    ...
    try:
        ...
        print(f"Validation error: {e}   Nothing added to gpt_responses. ChatGPT's response was: {reply}")
# %%
...
print(f"The predicted probability is: {median:.0f}%. With a 90% confidence interval of [{lower:.0f}%, {upper:.0f}%]")
# %%
